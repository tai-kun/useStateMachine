...
async function generate() {
  ...
  let twoSlashQueries = minimalTwoSlashQueries(
    twoslasher(GLOBALS + source, "ts", { vfsRoot: TEST_DIR })
  );
...
function minimalTwoSlashQueries(result) {
  return result.queries
    ...
    .map((q) => ({
      text: q.text,
      completions: q.completions?.map((c) => c.name),
    }));
}
...
function parseSource(source) {
  return source.split(EOL).reduce(
    (r, l) =>
      l.startsWith("import") || (r.body === "" && l.startsWith("/*")) // to include eslint comment
